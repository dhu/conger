...
void pretty_print(StockAggregateTuple tuple)
{
...
Status handleOutput(ptr<StreamEvent> event)
{
    if (event->_stream == Name("outputstream"))
    {
        ...
        {
            //offset += HEADER_SIZE;
            StockAggregateTuple *tuple =
                    (StockAggregateTuple *) &event->_bin_tuples[offset];
            offset += sizeof(StockAggregateTuple);
            pretty_print(*tuple);
        }
...
int main()
{
    Borealis::DequeueAggregate dequeue;
    dequeue.start();
}
