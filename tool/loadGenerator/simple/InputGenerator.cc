...
void stdArgs(int argc, const char *argv[])
{
    ...
    while ((option_char = getopt(argc, (char**)argv, "?hd:p:m:f:g:r:w:o:s:z:")) != EOF)
    {
        switch (option_char)
        {
            case 'f':
            ...
            case 'd':
                ...
                break;
            case 'g':
                BRIOParams::rate_multiplier = atof(optarg);
                break;
            case 'm':
...
int main(int argc, const char* argv[])
{
    ...
    NOTICE << "Starting application server on: " << address;

    BRInputGenerator input_generator(
