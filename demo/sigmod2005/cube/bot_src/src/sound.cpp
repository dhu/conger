...
{
  //addmsg(0, 2, SV_SOUND, n); playsound(n);
  NMSTL::ptr<cube_sound_tuple> ct = NMSTL::ptr<cube_sound_tuple>(new cube_sound_tuple());
  ct->clientnum = clientnum;
  ct->playsound_n = n;
  ct->isbot = 0;
  client_pending_message.push_back(std::make_pair((int)SV_SOUND, ct.dynamic_cast_to<borealis_tuple>()));
};
...
void botplaysound(int n, dynent *b)
{
  ...
  NMSTL::ptr<cube_sound_tuple> ct = NMSTL::ptr<cube_sound_tuple>(new cube_sound_tuple());
  ct->clientnum = BotManager.GetBotIndex(b);
  ct->playsound_n = n;
  ct->isbot = 1;
  client_pending_message.push_back(std::make_pair((int)SV_SOUND, ct.dynamic_cast_to<borealis_tuple>()));
