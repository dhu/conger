...
// Added by Rick: Enemy search for monsters.
// Since bots can play in DMSP we want to let the monsters shoot at them aswell
dynent *monstergetenemy(dynent *m)
{
     float nearestdist = 9999;
     dynent *bestenemy = NULL;
     
     if (m->enemy && (m->enemy->state == CS_ALIVE))
     {
          vec v = m->o;
          bool visible = los(m->o.x, m->o.y, m->o.z, m->enemy->o.x, m->enemy->o.y, m->enemy->o.z, v);
          if (visible) return m->enemy;
     }
     ...
     loopv(bots)
     {
          ...
          vdist(disttoenemy, vectoenemy, m->o, d->o);
          
          vec v = m->o;
          bool visible = los(m->o.x, m->o.y, m->o.z, d->o.x, d->o.y, d->o.z, v);

          if ((disttoenemy < nearestdist) && visible)
          {
     ...
     if ((player1->state == CS_ALIVE) && 0)
     {
          vdist(disttoenemy, vectoenemy, m->o, player1->o);
          
          vec v = m->o;
          bool visible = los(m->o.x, m->o.y, m->o.z, player1->o.x, player1->o.y, player1->o.z, v);
     ...
     return (bestenemy) ? bestenemy : player1;
}
...
void monsterrender()
{
    loopv(monsters) renderclient(monsters[i], false, monstertypes[monsters[i]->mtype].mdlname, monsters[i]->mtype==5, monstertypes[monsters[i]->mtype].mscale/10.0f, false);
};

// Added by Rick, called in dmsp mode when a bot joines the game
void monstercountgrow()
{
     spawnremain += (skill * 5);
     monstertotal += (skill * 5);
     nextmonster = std::max((lastmillis + 1000), nextmonster);
}
