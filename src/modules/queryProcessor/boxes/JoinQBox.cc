...
const int JoinQBox::LEFT = 0;
const int JoinQBox::RIGHT = 1;

void JoinQBox::setup_impl() throw( AuroraException )
{
    if ( get_num_inputs() != 2 )
...
ptr<JoinQBox::JoinBuffer> JoinQBox::create_buffer_by_parameter( int  stream_id )
{
    string name;
...
void JoinQBox::init_impl() throw( AuroraException )
{
...
void JoinQBox::run_impl( QBoxInvocation  &inv )
...
void JoinQBox::run_stream( QBoxInvocation &inv, int stream_id,
                           bool &outputted )
{
...
void JoinQBox::emit_tuple(EvalContext &ctxt)
{
...
void JoinQBox::produce_dont_emit_tuple(EvalContext &ctxt)
{
    EnqIterator ei = enq(0);
    ...
    if (_just_concat)
    {
      ...
      DEBUG << "Getting to the concatenation!";
      // this is not working properly
        uint8  *pos = output.get_body();
        memcpy(pos, left.get_body(), _left_body_size);
        pos += _left_body_size;
        memcpy(pos, right.get_body(), _right_body_size);
    if(left.get_array_flag() || right.get_array_flag())
      {
        int offset, field_index;
        //DataPath::DynamicArray *array_info;
        output.set_array_flag(1);
        TupleDescription out_td(get_out_schema(0));
        vector<SchemaField> schema = *out_td.get_schema_fields();
        for(uint x = 0; x < schema.size(); ++x)
          {
        if(schema[x].get_type() == DataType::ARRAY)
          {
            field_index = out_td.index_of_field(schema[x].get_name());
            offset = out_td.get_offset(field_index);
            //array_info = (DataPath::DynamicArray *)((*(int32 *) (((char *)output.get_data()) + offset)));
    ...
    {   Expression::eval_vector_into(_output_expressions,
                                     (char  *)output.get_body(),
                                     ctxt);
    }
}


void JoinQBox::JoinBuffer::join(const void *other_tuple,
...
void JoinQBox::JoinBuffer::join_no_emit(const void *other_tuple,
                                        Expression *predicate,
                                        bool &outputted)
{
    int other_stream_id = (_stream_id == LEFT) ? RIGHT : LEFT;
...
void JoinQBox::JoinBuffer::insert_tuple(const void *tuple_data)
{
...
void JoinQBox::JoinBuffer::clear()
{
    _buffer.clear();
}


void JoinQBox::TupleCountJoinBuffer::insert_tuple(const void *tuple_data)
{
    DEBUG << "Running TupleCountJoinBuffer::insertTuple("
          << _qbox->tuple_to_string(_stream_id, tuple_data)
          << ")";
    // Insert the tuple.
    JoinQBox::JoinBuffer::insert_tuple(tuple_data);
...
template<typename T>
T JoinQBox::FieldJoinBuffer<T>::field_value(const void *tuple_data)
{
    _ctxt.reset();
...
template<typename T>
void JoinQBox::FieldJoinBuffer<T>::insert_tuple(const void *tuple_data)
{
    DEBUG << "Running FieldJoinBuffer::insertTuple("
          << _qbox->tuple_to_string(_stream_id, tuple_data)
          << ")";
    // Insert the tuple.
    JoinQBox::JoinBuffer::insert_tuple(tuple_data);
...
template class JoinQBox::FieldJoinBuffer<int8>;
template class JoinQBox::FieldJoinBuffer<int16>;
template class JoinQBox::FieldJoinBuffer<int32>;
template class JoinQBox::FieldJoinBuffer<int64>;
template class JoinQBox::FieldJoinBuffer<single>;
template class JoinQBox::FieldJoinBuffer<double>;


void JoinQBox::set_pending_box_state(ptr<AbstractBoxState> packed_box)
{
    JoinBoxState* actual_packed_box = static_cast<JoinBoxState*>(packed_box.get());
    _pending_box_state.set(new JoinBoxState(*actual_packed_box));
}

ptr<AbstractBoxState> JoinQBox::pack_state(bool delta_only)
{

    INFO << "Packing state for JoinQBox named [" << get_name() << "]";
    ...
    ptr<JoinBoxState> join_box_state(new JoinBoxState(left_buffer,left_nb_tuples,right_buffer,right_nb_tuples));
...
void JoinQBox::unpack_state(ptr<AbstractBoxState> box_state)
{

    ptr<JoinBoxState> my_state = box_state.dynamic_cast_to<JoinBoxState>();
...
int JoinQBox::JoinBuffer::get_oldest()
{
...
int JoinQBox::JoinBuffer::dump(dynbuf& buf)
{
...
void JoinQBox::JoinBuffer::init(const dynbuf& buf, int nb_tuples)
{
    clear();
    int next_tuple_offset = 0;
...
NMSTL_SERIAL_DEFINE(JoinBoxState,     2000);
