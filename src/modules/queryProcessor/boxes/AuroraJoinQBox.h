#ifndef AURORA_JOIN_QBOX_H
#define AURORA_JOIN_QBOX_H
 ...
 * - Order by and slack.
 */
class AuroraJoinQBox : public QBox
{
  friend class AuroraJoinBoxState;
 ...
 private:
  ...
  class BufferElement
  {
    public:
      ...
      Timestamp get_timestamp() const { return _tuple.get_timestamp(); }

      int get_field_value() const { return _field_value; }

      // Get, Add, and Increment methods for selectivity counts. -- tatbul
      //
      int get_comparison_count() { return (_comparison_count); }
      int get_match_count() { return (_match_count); }

      void add_comparison_count(int count) { _comparison_count += count; }
      void add_match_count(int count) { _match_count += count; }

      void incr_comparison_count() { ++_comparison_count; }
      void incr_match_count() { ++_match_count; }
      //
      //
  ...
  /// Buffer for the two input streams to join
  class JoinBuffer
    {
    public:
      JoinBuffer( int stream_id, 
          AuroraJoinQBox *qbox ) :
    _stream_id( stream_id ),
    _qbox( qbox ),
    _tuple_size(_qbox->get_in_schema(_stream_id)->get_size() + HEADER_SIZE),
    ...
    _ctxt( 2 )
      ...
      /// Add tuple to buffer
      void insert_tuple( const void  *tuple_data );

      /// Trim buffer 
      virtual void trim_buffer(const void *tuple_data) {};

      /// Perform the join and emit tuple
      ...
      // Copy tuples to buffer and return nb tuples copied
      int dump(dynbuf& dst); 

      int get_oldest();


      /// Insert the tuples from src buffer into self buffer
      void init(const dynbuf& src, int nb_tuples); 

      // -- tatbul
      //
      BufferElement *back() { return(&(_buffer.back())); }

    protected:
      typedef list<BufferElement> BufferList;
      BufferList       _buffer;
      int              _stream_id;
      AuroraJoinQBox   *_qbox;
  ...
  class TupleCountJoinBuffer : public JoinBuffer
    {
    public:
      TupleCountJoinBuffer(int stream_id, 
               AuroraJoinQBox *qbox,
               size_t buffer_size) :
    ...
    private:
      ...
      // _buffer_size is the size from the first to the last tuple in this buffer.
      // equal to upsize for "left" buffer, downsize for "right" buffer.
      // only this one size is necessary as the only time it is used is while invalidating old tuples
      // from this buffer and that happens when a tuple is processed for the "other" stream (buffer)
      size_t _buffer_size;
    };
...
// -- magda the stuff below should be protected so subclasses can manipulate the buffers directly
 protected: 
 ...
 */
class AuroraJoinBoxState : public AbstractBoxState
{
 public:
  AuroraJoinBoxState() : 
    _left_nb_tuples(0), 
    _right_nb_tuples(0) 
    {}
  AuroraJoinBoxState(const dynbuf& left, 
             int nb_left, 
             const dynbuf& right, 
             int nb_right) :
  ...
  NMSTL_SERIAL_SUBCLASS(AuroraJoinBoxState, AbstractBoxState, << _left_buffer << _left_nb_tuples
