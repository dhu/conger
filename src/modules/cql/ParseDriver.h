...
public:
    ParseDriver();
    virtual ~ParseDriver();
    ParseContext parse(string query_name, string cql, string output_stream);
...
private:
    ...
    void handle_parse_contidtion(ParseContext& context, string& condition, pANTLR3_BASE_TREE node, string type);
