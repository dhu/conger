...
// --------------------------------------------------
/**
 * File options for client apps...
 * -f /path/to/xml/file
 */
void file_args(const int argc, const char* argv[], string& xml_query_file)
{
    const char* argv_copy[argc];
    for ( int i = 0; i < argc; i++)
    {
    ...
    optind = 0;
    opterr = 0;
    while ( (option_char = getopt(argc, (char**)argv_copy, file_args.c_str())) != EOF )
    {
    switch (option_char)
    {
    case 'f':
        xml_query_file = string(optarg);
        break;
    default:
        break;
    }
