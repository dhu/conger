...
ChunkMarshal   marshal;        // Client and I/O stream state.
...
//
void  ChunkMarshal::receivedCcddeltas(ccd_schema  *tuple)
{
//..............................................................................
  //NOTICE << "Out values: " <<  (int)*(tuple->frame) << ", " <<  (int) *(tuple->frame + 1) << ", " <<  (int)  *(tuple->frame + 2);
    ...
    if(tuple->final_bit == 1)
      {
      	/*InetAddress the_node("128.148.31.135:15000");
	ThreadClient client(the_node);
	Remote<QueryProcessor> qp = client.qp_for(MedusaID(the_node.as_string()));
	RPC<TSStats> cost = qp.get_statistics_name("Cost");
	RPC<TSStats> lat = qp.get_statistics_name("OutLatency");
	RPC<TSStats> sel = qp.get_statistics_name("Selectivity");
	DEBUG << "Cost: " << cost->as_string();
	DEBUG << "Selectivity: " << sel->as_string();
	DEBUG << "Out Latency: " << lat->as_string();
	*/
...
//
void  ChunkMarshal::sentCcdstream()
{
    int32        timestamp;
    Time         current_time;
    ostringstream os;
...
void ChunkMarshal::sentCcdchunked()
{
} 
////////////////////////////////////////////////////////////////////////////////
//
