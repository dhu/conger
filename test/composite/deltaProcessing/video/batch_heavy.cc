...
#define FRAME_START 175
#define FRAME_FINISH 200
...
BatchMarshal   marshal;        // Client and I/O stream state.

uint32  result = 0;
uint32  total  = BATCH_COUNT * BATCH_SIZE;
uint32  remain = FRAME_FINISH - FRAME_START+1;

uint8   expect = 0;
uint8   buffer[FRAME_WIDTH*FRAME_HEIGHT][FRAME_DEPTH];

vector<clock_t> sent_times;
vector<clock_t> recv_times;

bool loadImageCCD(string fileName)
{

  ifstream fin(fileName.c_str(), ios::in | ios::binary);
    fin.read((char *)buffer, FRAME_WIDTH*FRAME_HEIGHT*FRAME_DEPTH );
    if (fin.fail())
    {
...
//
void  BatchMarshal::receivedCcddeltas(ccd_schema  *tuple)
{
//..............................................................................
  //NOTICE << "Out values: " <<  (int)*(tuple->frame) << ", " <<  (int) *(tuple->frame + 1) << ", " <<  (int)  *(tuple->frame + 2);
	DEBUG << "Received frame!";
	recv_times.push_back(clock());
      tempImage = SDL_CreateRGBSurface(SDL_SWSURFACE, FRAME_WIDTH, FRAME_HEIGHT, 32, rmask, gmask, bmask, amask);
    if(tempImage == NULL)
      {
	cerr << "CreateRGBSurface failed: " << SDL_GetError() << endl;
    ...
    DEBUG << "Updated image! Frame number = " << ccdFrameNumber;
    if(ccdFrameNumber >= FRAME_FINISH)
      {
...
//
void  BatchMarshal::sentCcdstream()
{
    int32        timestamp;
    Time         current_time;
    ostringstream os;
...
//..............................................................................
    ...
    {   
        os << "/pro/borealis/data/demo/array/Charlotte" << std::setw(6) << std::setfill('0') << ccdFrameNumber << ".raw";
...
void BatchMarshal::sentCcdchunked()
{
} 
////////////////////////////////////////////////////////////////////////////////
//
...
//..............................................................................
    ...
    double avg_time;
    for(uint x = 0; x < recv_times.size()-1; ++x)
	{
		avg_time += recv_times[x] - sent_times[x];
		DEBUG << "Time difference: " << recv_times[x] - sent_times[x];
		//DEBUG << "Sent time: " << sent_times[x];
		//DEBUG << "Recv time: " << recv_times[x];
	}
	avg_time /= (double) recv_times.size();
    return(result);
}
