
#include "Lstest_Simple_AggregateMarshal.h"

using namespace Borealis;

const string ip = "127.0.0.1";    // client host ip.
const int port = 15000;           // client (and input) port.

uint32 batch_size = 10;           // Number of input tuples per batch.
uint32 base_sleep_time = 1000;

int time_counter = 0;

// send a random input
//
void  Lstest_Simple_AggregateMarshal::sentInput1()
{
    static long old_time = Time::now().to_usecs();
    static int count = 0;
    static uint32 sleep_time = base_sleep_time * 5;

    long now = Time::now().to_usecs();
    if (now - old_time > 1000000)
    {
        ++count;
        if (count == 4)
        {
            sleep_time /= 5;
        }
        if (count == 8)
        {
            sleep_time *= 5;
            count = 0;
        }
        old_time = now;
    }

    for (unsigned int i = 0; i < batch_size; ++i)
    {
        Input1 tuple;
        //tuple._data.timestamp = (Time::now().to_msecs());
        tuple._data.timestamp = time_counter++;
        tuple._data.value = (int)(drand48() * 10);
        DEBUG << "Sending Input1(" << tuple._data.timestamp << ", "
              << tuple._data.value  << ")";
        batchInput1(&tuple);
    }

    sendInput1(sleep_time);
}

// Print received tuples.
//
void Lstest_Simple_AggregateMarshal::receivedOutput1(AggTuple *tuple)
{
    static int count = 0;
    static long old_time = Time::now().to_usecs();

    ++count;
    long now = Time::now().to_usecs();
    if (now - old_time > 1000000)
    {
        cout << "Stream rate of Output1 is " << count << "." << endl;
        old_time = now;
        count = 0;
    }
    DEBUG << "Receiving Output1(" << tuple->timestamp << ", "
          << tuple->count  << ")";
}

////////////////////////////////////////////////////////////////////////////////
//
int main(int argc, const char *argv[])
{
    Lstest_Simple_AggregateMarshal marshal; // Client and I/O stream state.

    if (argc == 2)
        batch_size = atoi(argv[1]);

    marshal.open();

    // Send the first batch of tuples.  Queue up the next round with a delay.
    //
    marshal.sentInput1();

    // Run the client event loop.  Return only on an exception.
    //
    DEBUG  << "run the client event loop...";
    marshal.runClient();

}
