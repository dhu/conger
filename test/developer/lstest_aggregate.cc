...
#include "Lstest_AggregateMarshal.h"
...
// send a random input
//
void  Lstest_AggregateMarshal::sentInput1()
{
    ...
    for (unsigned int i = 0; i < batch_size; ++i)
    {
        ...
        tuple._data.value = (int)(drand48() * 10);
        DEBUG << "Sending Input1(" << tuple._data.timestamp << ", "
              << tuple._data.value  << ")";
        batchInput1(&tuple);
    }
...
// send a random input
//
void  Lstest_AggregateMarshal::sentInput2()
{
    static long old_time = Time::now().to_usecs();
    static int count = 0;
    ...
    for (unsigned int i = 0; i < batch_size; ++i)
    {
        ...
        tuple._data.timestamp = (Time::now().to_msecs());
        tuple._data.value = (int)(drand48() * 10);
        DEBUG << "Sending Input2(" << tuple._data.timestamp << ", "
              << tuple._data.value  << ")";
        batchInput2(&tuple);
    }
...
//
void Lstest_AggregateMarshal::receivedOutput1(IntTuple *tuple)
{
    static int count = 0;
    static long old_time = Time::now().to_usecs();
    ...
    if (now - old_time > 1000000)
    {
        cout << "Stream rate of Output1 is " << count << "." << endl;
        old_time = now;
        count = 0;
    }
    DEBUG << "Receiving Output1(" << tuple->timestamp << ", "
          << tuple->value  << ")";
}

// Print received tuples.
//
void Lstest_AggregateMarshal::receivedOutput2(AggTuple *tuple)
{
    static int count;
    static long old_time = Time::now().to_usecs();
    ...
    if (now - old_time > 1000000)
    {
        cout << "Stream rate of Output2 is " << count << "." << endl;
        old_time = now;
        count = 0;
    }
    DEBUG << "Receiving Output2(" << tuple->timestamp << ", "
          << tuple->count  << ")";
}
...
{
    Lstest_AggregateMarshal marshal;        // Client and I/O stream state.
