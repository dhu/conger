...
//
void LstestMarshal::receivedOutput1(IntTuple *tuple)
{
    static int count = 0;
    static long old_time = Time::now().to_usecs();
...
// Print the content of received tuples.
//
void LstestMarshal::receivedOutput2(IntTuple *tuple)
{
    static int count;
    static long old_time = Time::now().to_usecs();
...
{
    LstestMarshal marshal; // Client and I/O stream state.
