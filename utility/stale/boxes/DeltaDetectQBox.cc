...
void DeltaDetectQBox::setup_impl() throw (AuroraException)
{
  int dim_size;
  ...
  vector<SchemaArray::DimensionInformation> theDimInfo;
    ...
    DEBUG << "DeltaDetectQBox input schema " << get_in_schema(0)->as_string();
    DEBUG << "DeltaDetectQBox output schema " << get_out_schema(0)->as_string();
...
DeltaDetectQBox::~DeltaDetectQBox()
{
  _cached_array->modify_array_reference(-1);
}

void DeltaDetectQBox::init_impl() throw (AuroraException)
{
...
void DeltaDetectQBox::run_impl(QBoxInvocation& inv) throw (AuroraException)
{
    uint32 *element;
...
void DeltaDetectQBox::set_pending_box_state(ptr<AbstractBoxState> packed_box)
{
    ...
    _pending_box_state = packed_box.dynamic_cast_to<DeltaDetectBoxState>();
}

ptr<AbstractBoxState> DeltaDetectQBox::pack_state(bool delta_only)
{
  INFO << "Packing state for DeltaDetectQBox named [" << get_name() << "]";
  ptr<DeltaDetectBoxState> filter_box_state(new DeltaDetectBoxState());
  return filter_box_state;
}


void DeltaDetectQBox::addCoord(uint32 dim, uint32 n, uint32 &g)
{
...
float DeltaDetectQBox::calculate_percent_changed(uint32 chunk_number)
{
...
void DeltaDetectQBox::map_chunk(int chunk_number)
{
...
uint8 DeltaDetectQBox::getValue( unsigned int x, unsigned int y, unsigned int z, char *array)
{
...
void DeltaDetectQBox::setValue( unsigned int x, unsigned int y, unsigned int z, char *array, uint8 theValue)
{
  int offset = (_array_dimensions[0]*y+x)*_array_dimensions[2]+z;
...
DynamicArray *DeltaDetectQBox::create_new_chunk(int chunk_number)
{
