 ...
 * - Order by and slack.
 */
class MergeQBox : public QBox
{
    
    friend class MergeBoxState;

    // SMergeQBox is a kind of MergeBox. It manipulates the _buffer directly
    friend class SMergeQBox;
  ...
  private:
    bool is_less_than(pair<single, single>, pair<single, single>);
    ...
    {
      public:
        MergeBuffer( int stream_id, MergeQBox *qbox ) :
    ...
    friend class MergeBuffer;

    // Subclass for ORDER_BY_VALUES
    class TupleCountMergeBuffer : public MergeBuffer
    {
      public:
        TupleCountMergeBuffer(int stream_id, MergeQBox *qbox,
                             size_t buffer_size) :
            MergeBuffer(stream_id, qbox),
    ...
    template<typename T>
    class FieldMergeBuffer : public MergeBuffer
    {
      public:
        FieldMergeBuffer(int stream_id, MergeQBox *qbox,
                        ptr<Expression> field,
//                        size_t buffer_size) :
                        T buffer_size) :
            MergeBuffer(stream_id, qbox), _field(field),
            _buffer_size(buffer_size)
        {
//            if (!_field->is<int32>())
...
//        size_t _buffer_size;
 ...
 protected: // -- magda the stuff below should be protected so subclasses can manipulate the buffers directly
    ...
    ptr<MergeBuffer> create_buffer_by_parameter(int stream_id);

    // Left and right join buffers.
    ptr<MergeBuffer> _buffer[2];
    ...
    // Merge selectivity stats. -- tatbul
    //
 ...
 */
class MergeBoxState : public AbstractBoxState
{
 public:
  MergeBoxState() : _left_nb_tuples(0), _right_nb_tuples(0) {}
  MergeBoxState(const dynbuf& left, int nb_left, const dynbuf& right, int nb_right) :
  ...
  NMSTL_SERIAL_SUBCLASS(MergeBoxState, AbstractBoxState, << _left_buffer << _left_nb_tuples
