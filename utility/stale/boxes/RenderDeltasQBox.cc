...
void RenderDeltasQBox::load_map()
{
  MAP_WIDTH = 640;
  MAP_HEIGHT = 480;
  MAP_DEPTH = 3;
  ...
  _filename = "/pro/borealis/data/demo/array/us_map.raw";
    ...
    _map_cache = DynamicArray::allocate_array(MAP_SIZE);
    _map_cache->set_count_size(1,1);
    memcpy(_map_cache->first_position(), _original_map, MAP_SIZE);
}

void RenderDeltasQBox::setup_impl() throw (AuroraException)
{
    ...
    {   Throw(aurora_typing_exception,
              "RenderDeltas requires exactly one input stream (not " +
    ...
    for(unsigned int x = 0; x < schema._field.size(); ++x)
      {
        ...
        if(curr_field.get_type() == DataType::ARRAY && array_rewrite == true)
            FATAL << "RenderDeltas currently supports only one array per schema.";
      ...
      if(!array_rewrite) // no array found
        FATAL << "RenderDeltas requires at least one array.";
    ...
    */
    ...
    DEBUG << "RenderDeltasQBox input schema " << get_in_schema(0)->as_string();
    DEBUG << "RenderDeltasQBox output schema " << get_out_schema(0)->as_string();
}

void RenderDeltasQBox::init_impl() throw (AuroraException)
{
...
void RenderDeltasQBox::run_impl(QBoxInvocation& inv) throw (AuroraException)
{
    ...
    updates = update_cache(positions);
    render_points(_map_cache, updates);
        _map_cache->modify_array_reference(1);
        memcpy(enq(0).tuple(), data, _output_tuple_size);
        // set element val here
        element = (uint32 *)((uint8 *)enq(0).tuple() + _out_array_offset);
        *element  = (uint32) _map_cache;
        ++(enq(0));
...
ship_cache RenderDeltasQBox::update_cache(DynamicArray *updates)
{
    ...
    while(current_rec != 0  ) 
    {
        ...
        id = current_rec->id;
        DEBUG << "Dealing with id: " << id;
        if(0 < sp.x && sp.x < MAP_WIDTH && 0 < sp.y && sp.y < MAP_HEIGHT)
          {
            DEBUG << "    point in range!";
            // erase the old one; replace it with a new one
            if(_cached_ships.find(id) != _cached_ships.end())
              {
            erase_record(id, _cached_ships[id]);
              }
            _cached_ships[id] = sp;
            new_ones[id] = sp;
          }
...
void RenderDeltasQBox::render_points(DynamicArray *out_array, ship_cache points)
{
    ship_cache::iterator cache_pos = points.begin();
    ship_position coordinate;
    uint32 start_x, start_y;
    
    while(cache_pos != points.end())
    {
...
void RenderDeltasQBox::draw_id(DynamicArray *out_array, string id, uint32 start_x, uint32 start_y)
{
    try
    {
...
void RenderDeltasQBox::fill_point(DynamicArray *array, uint32 x, uint32 y)
{
...
void RenderDeltasQBox::set_pixel_red(uint32 x, uint32 y, DynamicArray *the_out)
{
...
void RenderDeltasQBox::erase_record(uint32 id, ship_position record)
{
  DEBUG << "Erasing id: " <<id << " starting at " << record.x << ", " << record.y;
  uint32 start_x, start_y, img_width, img_height, effective_width, offset, jump_size;
  uint8 *src, *dst;
  ...
  if(start_x + img_width > MAP_WIDTH)
   ...
   offset = (MAP_WIDTH*start_y + start_x)*MAP_DEPTH;
   dst = _map_cache->first_position() + offset;
   src = _original_map + offset;
   jump_size = (MAP_WIDTH - effective_width)*MAP_DEPTH;

   for(uint32 y = 0; y < img_height; ++y)
     {
       for(uint32 x = 0; x < img_width; ++x)
     for(uint32 z = 0; z < MAP_DEPTH; ++z)
       {
         *dst = *src;
         ++dst;
         ++src;
...
void RenderDeltasQBox::set_pending_box_state(ptr<AbstractBoxState> packed_box)
{
    //RenderDeltasBoxState* actual_packed_box = static_cast<RenderDeltasBoxState*>(packed_box.get());
    //_pending_box_state.set(new RenderDeltasBoxState(*actual_packed_box));
    _pending_box_state = packed_box.dynamic_cast_to<RenderDeltasBoxState>();
}

ptr<AbstractBoxState> RenderDeltasQBox::pack_state(bool delta_only)
{
  INFO << "Packing state for RenderDeltasQBox named [" << get_name() << "]";
  ptr<RenderDeltasBoxState> filter_box_state(new RenderDeltasBoxState());
